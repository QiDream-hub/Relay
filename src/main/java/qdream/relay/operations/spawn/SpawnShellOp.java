 ...
 */
public class SpawnShellOp extends Instruction {
    ...
    /**
     * 将 ListData 转换为 Executable 列表
     */
    private List<Executable> convertProgram(ListData listData) {
        java.util.List<Executable> program = new java.util.ArrayList<>();
        java.util.List<Executable> innerList = listData.getValue();

        for (Executable exe : innerList) {
            program.add(exe);
        }
