...
public class PickupItemOp extends Spell {
    ...
    public void execute(StateMachine executor) {
        ...
        ServerLevel containerLevel = Relay.getWorld(blockEntityData.getWorldId());
        ...
        if (containerLevel.isClientSide() || entityLevel.isClientSide()) {
            executor.triggerMishap(id + " 错误的实体或错误的容器");
            return;
        }

        // 获取 BlockEntity（使用容器所在的世界）
        var blockEntity = blockEntityData.getBlockEntity(containerLevel);
