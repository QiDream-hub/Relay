 ...
 */
public class BlockType extends Data {
    // 方块位置
    ...
    public BlockType(BlockPos blockPos, String worldId, BlockState blockStateRef) {
     ...
     */
    public static BlockType from(BlockState blockState, BlockPos pos, Level world) {
        if (blockState == null) {
            return new BlockType(null, null, null);
        }
        String worldId = world.dimension().registry().toString();
        BlockPos immutablePos = pos.immutable();
        return new BlockType(immutablePos, worldId, blockState);
    }

    /**
     * 从 BlockPos 和世界 ID 创建 BlockType（用于反序列化，不保持引用）
     */
    public static BlockType fromBlockPos(BlockPos blockPos, String worldId) {
        return new BlockType(blockPos, worldId, null);
    }
    ...
    public Data fromNbt(CompoundTag tag) {
        ...
        return BlockType.fromBlockPos(blockPos, worldId);
    }
